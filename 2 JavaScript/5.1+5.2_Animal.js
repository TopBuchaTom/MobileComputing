...
//  ,-> statt class eine function                                                           ,-> sorgt dafür, dass this auf ein leeres Objekt initialisiert wird vom Typ der umgebenden Funktion -> bekommt neues Animal, wo genau diese Eigenschaften gesetzt sind 
...
function Dog(name, size, breed) {
    //Weitere Alternative zu call()
        ...
        // ,-> = Hilfsvariable: base verweist auf Animal-Funktion
    ...
    //     ,-> Animal-Funktion über Variable aufrufen; Trick: immer wenn man Variable hat, die auf eine andere Funktion verweist, dann wird auto. das this, der Funktion (hier Dog) in dem die Variable definiert ist, mitgegeben beim Aufruf
    this.base(name, size);
...
//Dog erbt von Animal - Vererbungsbeziehung herstellen:
...
class Animal {  //Klasse Animal als Überklasse für Dog & Cat --> Dog & Cat erben von Animal
  ...
  static instanceCount = 0; //wird jedes Mal um eins erhöht, wenn der Konstruktor aufgerufen wird
}
...
//          '-> dadurch erbt man alles, was in Animal definiert ist --> bekommt auch getName()
  constructor(name, size, breed) {  //=Konstruktor; breed = Rasse
      ...
      // '-> Konstruktor von Oberklasse Animal aufrufen/super führt Super-Konstruktor (=Konstruktor von dem man erbt = Konstruktor von Animal) aus
      this.breed = breed;
  }
