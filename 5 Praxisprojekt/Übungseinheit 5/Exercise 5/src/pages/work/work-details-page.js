...
export class WorkDetailsPage extends LitElement {
        ...
        <page-content>
          <form>
            ...
            <ion-item>
              ...
              <ion-icon icon=${businessOutline} @click=${() => this._selectLocation(OrganizationsPage.name)}></ion-icon>
              <ion-icon icon=${personOutline} @click=${() => this._selectLocation(PersonsPage.name)}></ion-icon>
            </ion-item>
            <ion-item>
  ...
  _selectLocation(type) {
    routerService.pushDialog(type, { selected: this._workEntry.location });
  }
}
